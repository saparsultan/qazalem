...
const AboutCountryPageSubClient = ({ lng }) => {
  const { subSlug } = useParams();
  const router = useRouter();
  const { data, isLoading, isSuccess } = useQuery({
    queryKey: ["infoAboutCountrySubContent"],
    queryFn: async () => {
      const { data } = await InformationService.getAboutCountrySubContent(
        subSlug,
        lng,
      );
      return data;
    ...
    <>
      <div className="about-country-page__head">
        ...
        <h2 className="title title-h2">
          {!isLoading && isSuccess && data?.subcategory?.name}
        </h2>
      </div>
          ...
          <div className="about-country-content__content">
            <div
              dangerouslySetInnerHTML={{
                __html: !isLoading && isSuccess && data?.body_text,
              }}
...
export default AboutCountryPageSubClient;
